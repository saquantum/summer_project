...
class DataFetcherManager:
    ...
    def __init__(
        ...
        if use_selenium: # 只有當明確要求使用 Selenium 時才嘗試初始化
            ...
            try:
                ...
                self._driver = webdriver.Chrome(options=chrome_options) # 直接使用，需 ChromeDriver 在 PATH
            except Exception as e:
                logger.error(f"[DataFetcherManager] 初始化 Selenium WebDriver 失敗: {e}. 請確保 ChromeDriver 已正確安裝並在 PATH 中，或考慮使用 webdriver-manager。")
                self._use_selenium = False # 初始化失敗，則將 use_selenium 設回 False
                self._driver = None
                logger.warning("由於 WebDriver 初始化失敗，此 DataFetcherManager 實例將僅使用 requests 模式。")
    ...
    def fetcher(self) -> Result[BeautifulSoup]:
        logging.info("[DataFetcherManager] 開始抓取主頁面 URL: %s (模式: %s)", self._url, "Selenium" if self._use_selenium and self._driver else "requests")
        try:
            html_content_str = ""
            if self._use_selenium and self._driver: # 確保 driver 也被成功初始化
                self._driver.get(self._url)
                self._selenium_wait_for_element("tag name", "body", timeout=random.uniform(5, 10))
                html_content_str = self._driver.page_source
                try:
                    abs_save_dir_path_str = FileAndFolderManager.create_folder(self.RAW_HTML_SAVE_DIR)
                ...
                if self._use_selenium and not self._driver: # 原本想用 selenium 但 driver 初始化失敗
                    logger.warning("Selenium driver 未成功初始化，將嘗試使用 requests 模式抓取主頁面。")
    ...
    def get_html_from_url(self, target_url: str) -> Result[str]:
        logger.info("[DataFetcherManager] 開始抓取獨立文章 URL: %s (模式: %s)", target_url, "Selenium" if self._use_selenium and self._driver else "requests")
        try:
            ...
            if self._use_selenium and self._driver:
                ...
                try:
                    ...
                    logger.error(f"儲存 Selenium 網頁原文失敗 for {target_url}: {save_e}", exc_info=True)
                # --- MODIFICATION END ---
            else:
                html_content = self._download_html(target_url) # _download_html 內部會儲存檔案
            return Result.ok(html_content)
        except requests.exceptions.HTTPError as e:
            logger.error("抓取獨立文章 HTTP 錯誤 %s: %s (狀態碼: %s)", target_url, e, e.response.status_code if e.response else "N/A", exc_info=False)
            return Result.fail(f"HTTP 錯誤 {e.response.status_code if e.response else 'N/A'} 於 {target_url}: {str(e)}")
        except requests.exceptions.RequestException as e:
            logger.error("抓取獨立文章 Request 錯誤 %s: %s", target_url, e, exc_info=True)
            return Result.fail(f"請求錯誤於 {target_url}: {str(e)}")
        except Exception as e: # 其他所有錯誤，包括 Selenium 的
            logger.error("抓取獨立文章時發生未預期錯誤 %s: %s", target_url, e, exc_info=True)
    ...
    def _download_html(self, target_url: str) -> str:
        ...
        resp = self._session.get(target_url, headers=headers, timeout=random.uniform(5, 10))
        ...
        try:
            ...
            if match_header_charset:
                ...
                if header_encoding != 'iso-8859-1': # 忽略不可靠的 iso-8859-1
                    logger.info(f"從 HTTP header Content-Type 找到編碼: {header_encoding} for URL: {target_url}")
                    final_encoding = header_encoding
            
            # 2. 如果 header 中沒有或不可靠，嘗試從 HTML meta 標籤偵測
            if not final_encoding:
                detected_encoding = self._detect_charset(resp.content) # resp.content is bytes
                logger.info(f"透過 _detect_charset 偵測到編碼: {detected_encoding} for URL: {target_url} (Header編碼為: {resp.encoding or '未指定'})")
                final_encoding = detected_encoding

            # 3. 如果以上都失敗，使用 requests 的 apparent_encoding
            if not final_encoding:
                final_encoding = resp.apparent_encoding
                logger.info(f"使用 apparent_encoding: {final_encoding} for URL: {target_url}")

            resp.encoding = final_encoding # 設定 response 物件的編碼，resp.text 會用這個
            # --- MODIFICATION END ---
        except Exception as e:
            logger.warning(f"編碼設定/偵測過程中發生錯誤 for URL {target_url} (錯誤: {e})，將依賴 requests 預設的編碼處理。")
            final_encoding = resp.encoding if resp.encoding else resp.apparent_encoding # 確保 final_encoding 有值
        ...
        try:
            ...
            # --- END MODIFICATION ---
            with open(full_file_path, "w", encoding=encoding_to_save, errors='replace') as f: # Add errors='replace'
                f.write(html_content_str)
            logger.info(f"網頁原文已儲存到: {full_file_path} (編碼: {encoding_to_save})")
        except Exception as e:
            file_path_for_log = locals().get('full_file_path', f"未定義路徑於目錄({self.RAW_HTML_SAVE_DIR})")
            logger.error(f"儲存網頁原文到 {file_path_for_log} 失敗: {e}", exc_info=True)
        
        return html_content_str

    def _configure_fetch_retry(self, *, total: int, backoff: float, 
                               status_forcelist: list[int], 
                               allowed_methods: list[str],    
    ) -> None:
        # ... (此方法保持不變) ...
        retry_strategy = Retry( total=total, backoff_factor=backoff, status_forcelist=status_forcelist, allowed_methods=allowed_methods,)
    ...
    def _detect_charset(html_bytes: bytes) -> str:
        ...
        for enc in encodings_to_try:
            try:
                temp_html_text_for_meta_parsing = html_bytes.decode(enc, errors='ignore') 
        ...
        if not decoded_successfully:
            logger.warning("無法使用常見編碼初步解碼 HTML bytes 以偵測 charset，將返回預設 utf-8")
            return 'utf-8'
        ...
        try:
            ...
            meta_content_type_tag = soup.find("meta", attrs={"http-equiv": re.compile("content-type", re.I)})
            if meta_content_type_tag and meta_content_type_tag.get("content"):
                ...
                if match:
                    ...
                    logger.debug(f"透過 meta http-equiv content-type 找到編碼: {found_encoding}")
                    return found_encoding
            
            logger.debug("在 meta 標籤中未找到明確的 charset。")
            return "utf-8" 
        except Exception as e: 
            logger.warning(f"解析 HTML meta 標籤以偵測 charset 時發生錯誤: {e}。返回預設 utf-8。")
            return "utf-8"
    # --- END MODIFICATION ---

    def _selenium_wait_for_element(self, by_method_str: str, value: str, timeout: float = 10): # <<< MODIFIED: Parameter name and type hints
        ...
        if not self._driver:
            logger.error("Selenium driver 尚未初始化 (在 _selenium_wait_for_element 中檢查)。")
            # --- MODIFIED: Raise a more specific or descriptive error ---
            raise ReferenceError("Selenium driver not initialized before calling _selenium_wait_for_element.")
            # --- END MODIFICATION ---
