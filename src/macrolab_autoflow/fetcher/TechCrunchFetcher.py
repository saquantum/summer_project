...
class TechCrunchFetcher(BaseFetcher[BeautifulSoup, List[Dict]]):
    ...
    def __init__(self, use_selenium: bool = False):
        self._manager = DataFetcherManager(
            url="https://techcrunch.com/", use_selenium=use_selenium
        )
        ...
        try:
            ...
            for selector in article_elements_selectors:
                ...
                for el in found_elements:
                    ...
                    if el.select_one(
                        "h2 a, h3 a, a.post-block__title__link, a.wp-block-post-title__link"
                    ):
                        article_elements.append(el)

                if article_elements:
                    logger.info(
                        f"在主頁使用 selector '{selector}' 初步找到了 {len(article_elements)} 個潛在文章區塊。"
                    )
                    break

            if not article_elements:
                article_elements = raw.select(
                    "div.loop-card--post-type-post"
                )  # 根據您提供的 HTML，這個可能更準確
                if article_elements:
                    logger.info(
                        f"使用 selector 'div.loop-card--post-type-post' 找到了 {len(article_elements)} 個潛在文章區塊。"
                    )
            ...
            if not article_elements:
                ...
                if article_elements:
                    logger.info(
                        f"使用通用 selector 'article' 找到了 {len(article_elements)} 個潛在文章區塊。"
                    )
            ...
            if not article_elements:
                logger.error(
                    "解析失敗：在主頁面找不到任何可識別的文章區塊。請檢查主頁面文章列表的 CSS selector。"
                )
                return Result.fail(
                    "解析失敗：找不到任何文章區塊，請檢查主頁面文章列表的 CSS selector 是否更新"
                )

            articles: List[Dict] = []
            for idx, article_element in enumerate(article_elements):
                logger.debug(
                    f"正在處理第 {idx+1}/{len(article_elements)} 個主頁文章區塊..."
                )
                title = "N/A"
                url = None
                published_at = None
                ...
                if not title_link_el:
                    ...
                    for link_idx, link in enumerate(potential_links):
                        ...
                        is_article_link = (
                            ...
                            and "/tag/" not in href
                            and "tcrpmg.wordpress.com" not in href
                        ...
                        if is_article_link:
                            ...
                            if len(link_text) > 15 and not link.find_parent(
                                ["div", "span"],
                                class_=lambda x: x
                                and "byline" in x
                                or "cat" in x
                                ...
                                title_link_el = link
                                logger.debug(
                                    f"標題連結透過 fallback 邏輯找到 (第 {link_idx+1} 個潛在連結): {link_text}"
                                )
                                break
                ...
                if title_link_el:
                    ...
                    logger.warning(
                        f"第 {idx+1} 個文章區塊未找到有效標題連結。HTML 片段: {str(article_element)[:200]}..."
                    )
                ...
                if category_el:
                    ...
                    logger.warning(
                        f"第 {idx+1} 個文章區塊 (標題: {title}) 未找到分類。"
                    )
                # +++ MODIFICATION END +++
                ...
                if time_el and time_el.has_attr("datetime"):
                    ...
                    logger.warning(
                        f"第 {idx+1} 個文章區塊 (標題: {title}) 未找到發佈時間。"
                    )
                ...
                if url and url.startswith("https://techcrunch.com/"):
                    ...
                    if content_html_res.is_success() and content_html_res.value:
                        ...
                        for sel in content_container_selectors:
                            ...
                            if content_container:
                                logger.debug(
                                    f"文章內容容器使用 selector '{sel}' 找到 for URL: {url}"
                                )
                                break

                        if content_container:
                            paragraphs = content_container.find_all(
                                [
                                    "p",
                                    "h1",
                                    "h2",
                    ...
                    logger.warning(
                        f"URL '{url}' 不是 TechCrunch 官方 URL 或無效，跳過內容抓取。"
                    )
            ...
            if not articles:
                logger.warning(
                    "雖然可能找到了文章區塊，但未能從中提取任何有效的文章資訊（標題/URL）。"
                )
                return Result.fail(
                    "解析失敗：找到了文章區塊，但無法從中提取有效的標題和連結。"
                )
...
if __name__ == "__main__":
    ...
    if articles:
        ...
        for i, article_item in enumerate(articles):
            ...
            print(
                f"內容 (前 1000 字元): {content_preview[:1000] + ('...' if len(content_preview) > 100 else '')}"
            )
            print("---------------------------------")
        # --- MODIFICATION END ---
    else:
